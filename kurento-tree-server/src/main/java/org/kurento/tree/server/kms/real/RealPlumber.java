...
public class RealPlumber extends Plumber implements RealElement {
	...
	public RealPlumber(RealPipeline pipeline) {
		...
		if (USE_WEBRTC_AS_PLUMBER) {
			endpoint = new WebRtcEndpoint.Builder(pipeline.getMediaPipeline())
					.build();
		} else {
			endpoint = new PlumberEndpoint.Builder(pipeline.getMediaPipeline())
					.build();
		}
	...
	public void link(Plumber plumber) {
		...
		if (USE_WEBRTC_AS_PLUMBER) {
			...
			log.debug("Connecting webRtcs from Kms {} to Kms {}",
					this.getPipeline().getKms(),
					plumber.getPipeline().getKms());

			thisWebRtc.addOnIceCandidateListener(
					(e) -> otherWebRtc.addIceCandidate(e.getCandidate()));
			otherWebRtc.addOnIceCandidateListener(
					(e) -> thisWebRtc.addIceCandidate(e.getCandidate()));

			thisWebRtc.processAnswer(
					otherWebRtc.processOffer(thisWebRtc.generateOffer()));
			...
			log.debug("Connecting plumber to adress:" + address + " port:"
					+ port);
